...
def cleanup_quota_temp_html_files() -> tuple[int, int]:
    ...
    Only files created by Open HTML are removed. User-selected saved
...
DUMPIT_PATCH_STATUS_FAILED_FILE_MISSING = "FAILED_FILE_MISSING"
DUMPIT_PATCH_FAILED_STATUSES = {
    DUMPIT_PATCH_STATUS_FAILED_NOT_FOUND,
    DUMPIT_PATCH_STATUS_FAILED_AMBIGUOUS,
    DUMPIT_PATCH_STATUS_FAILED_FILE_MISSING,
}
DUMPIT_PATCH_HUNK_RE = re.compile(
...
class UnifiedPatchFile:
    old_path: str
    new_path: str
    hunks: tuple[UnifiedPatchHunk, ...]


@dataclass(slots=True, frozen=True)
class DumpItPatchHunkResult:
    file_path: str
    hunk_index: int
    status: str
    ...
    def failed(self) -> bool:
        return self.status in DUMPIT_PATCH_FAILED_STATUSES


@dataclass(slots=True)
class DumpItPatchFileResult:
    rel_path: str
    target_path: Path
    existed: bool
    encoding: str
    ...
    before_text: str
    after_text: str | None
    hunk_results: tuple[DumpItPatchHunkResult, ...]
    action: str = "modify"

    @property
    def failed(self) -> bool:
        return any(item.failed for item in self.hunk_results)
    ...
    def changed(self) -> bool:
        ...
        if self.action == "delete":
            return bool(self.existed)
        return (self.after_text or "") != self.before_text


@dataclass(slots=True)
class DumpItPatchPlan:
    root: Path
    patch_path: Path
    strip_level: int
    reverse: bool
    patch_directory: str | None
    files: tuple[DumpItPatchFileResult, ...]
    parse_errors: tuple[str, ...] = ()

    @property
    def total_hunks(self) -> int:
        return sum(len(item.hunk_results) for item in self.files)

    @property
    def failed_hunks(self) -> int:
        return sum(1 for item in self.files for hunk in item.hunk_results if hunk.failed)

    @property
    def failed(self) -> bool:
        return bool(self.parse_errors or self.failed_hunks)
    ...
    def status_summary(self) -> str:
        return (
            ...
            f"changed_files={self.changed_files}, failed_hunks={self.failed_hunks}"
        )


@dataclass(slots=True, frozen=True)
class DumpItPatchApplyPlan:
    root: Path
    strip_level: int
    patch_directory: str | None
    plan: DumpItPatchPlan
    attempts: tuple[str, ...]


@dataclass(slots=True)
class _DumpItPatchTextDocument:
    lines: list[str]
    newline: str
    encoding: str
    final_newline: bool
...
def parse_unified_patch(patch_path: Path) -> tuple[UnifiedPatchFile, ...]:
    """Parse a normal unified diff into file/hunk records.

    The parser intentionally supports text hunks only. Binary patch payloads are
    rejected by the DumpIt engine and remain Git engine territory.
    """
    ...
    current_hunks: list[UnifiedPatchHunk] = []
    i = 0

    def flush_file() -> None:
        nonlocal current_old_path, current_new_path, current_hunks
        if current_old_path is not None and current_new_path is not None:
            files.append(
                UnifiedPatchFile(
    ...
    while i < len(raw_lines):
        ...
        if line.startswith("GIT binary patch") or line.startswith("Binary files "):
            raise ValueError("DumpIt apply supports text unified diff hunks only; use Git apply for binary patches.")

        if line.startswith("--- "):
            if i + 1 < len(raw_lines) and raw_lines[i + 1].startswith("+++ "):
                flush_file()
                current_old_path = _normalize_patch_raw_path(line[4:].strip())
                current_new_path = _normalize_patch_raw_path(raw_lines[i + 1][4:].strip())
                i += 2
                continue

        if current_old_path is not None and current_new_path is not None and line.startswith("@@ "):
    ...
    flush_file()
    if not files:
        raise ValueError("No text file hunks were found in this patch.")
    return tuple(files)


def _hunk_old_lines(hunk: UnifiedPatchHunk) -> list[str]:
    return [line[1:] for line in hunk.lines if line and line[0] in {" ", "-"}]


def _hunk_new_lines(hunk: UnifiedPatchHunk) -> list[str]:
    return [line[1:] for line in hunk.lines if line and line[0] in {" ", "+"}]


def _effective_hunk_blocks(hunk: UnifiedPatchHunk, *, reverse: bool) -> tuple[int, list[str], list[str]]:
    if reverse:
        return int(hunk.new_start), _hunk_new_lines(hunk), _hunk_old_lines(hunk)
...
def _effective_patch_paths(patch_file: UnifiedPatchFile, *, reverse: bool) -> tuple[str, str, str, bool, bool]:
    ...
    target_raw = effective_new if _normalize_patch_raw_path(effective_new) != "/dev/null" else effective_old
    is_create = _normalize_patch_raw_path(effective_old) == "/dev/null"
    is_delete = _normalize_patch_raw_path(effective_new) == "/dev/null"
    return effective_old, effective_new, target_raw, bool(is_create), bool(is_delete)


def _read_dumpit_patch_text_document(path: Path, *, default_newline: str | None = None) -> _DumpItPatchTextDocument:
    raw = path.read_bytes()
    encoding = "utf-8"
    text = ""
    candidates = ("utf-8-sig", "utf-8", "cp1252") if raw.startswith(b"\xef\xbb\xbf") else ("utf-8", "cp1252")
    for candidate in candidates:
        try:
    ...
    newline = "\r\n" if crlf_count and crlf_count >= max(1, lf_count - crlf_count) else (default_newline or "\n")
    final_newline = text.endswith(("\n", "\r"))
    return _DumpItPatchTextDocument(
...
def _new_dumpit_patch_text_document(*, default_newline: str | None = None) -> _DumpItPatchTextDocument:
    return _DumpItPatchTextDocument(
        lines=[],
        newline=default_newline or os.linesep or "\n",
        encoding="utf-8",
        final_newline=True,
        before_text="",
    )
...
def _render_dumpit_patch_lines(lines: list[str], *, newline: str, final_newline: bool) -> str:
    if not lines:
        return newline if final_newline else ""
...
def _match_block_at_line(lines: list[str], start_line: int, block: list[str]) -> int | None:
...
def _trim_single_trailing_empty_context(old_block: list[str], new_block: list[str]) -> tuple[list[str], list[str]] | None:
    """Return EOF-safe blocks when a patch carries one empty context line past EOF.

    Unified diffs may contain a trailing empty context line represented as a
    single space in the patch.  If the target file ends immediately before that
    synthetic context line, strict matching is allowed to fail even though the
    intended hunk is exact.  This helper only trims one paired empty context
    line from OLD and NEW; it does not ignore whitespace or partial context.
    """
    if not old_block or not new_block:
        return None
    if old_block[-1] != "" or new_block[-1] != "":
        return None
    return list(old_block[:-1]), list(new_block[:-1])


def _find_unique_eof_context_block(lines: list[str], block: list[str]) -> int | None:
    matches = _find_exact_block(lines, block)
    if len(matches) != 1:
        return None
    idx = matches[0]
...
def _resolve_dumpit_hunk(
    *,
    ...
    if eof_blocks is not None:
        ...
        eof_old_idx = _find_unique_eof_context_block(lines, eof_old_block)
        if eof_old_idx is not None:
            updated = list(lines)
            updated[eof_old_idx:eof_old_idx + len(eof_old_block)] = eof_new_block
            return (
                DumpItPatchHunkResult(
                    file_path=rel_path,
                    hunk_index=hunk_index,
                    status=DUMPIT_PATCH_STATUS_APPLICABLE_RELOCATED_EOF_CONTEXT,
                    line_no=eof_old_idx + 1,
                    detail="old block found once at EOF after trimming one trailing empty context line",
                ),
                updated,
            )

        eof_new_idx = _find_unique_eof_context_block(lines, eof_new_block)
        if eof_new_idx is not None:
            return (
                DumpItPatchHunkResult(
                    file_path=rel_path,
                    hunk_index=hunk_index,
                    status=DUMPIT_PATCH_STATUS_ALREADY_APPLIED,
                    line_no=eof_new_idx + 1,
                    detail="new block already present at EOF after trimming one trailing empty context line",
                ),
                lines,
            )
...
def build_dumpit_patch_plan(
    *,
    ...
    for patch_file in patch_files:
        _effective_old, _effective_new, target_raw, is_create, is_delete = _effective_patch_paths(patch_file, reverse=reverse)
        rel = _strip_patch_path(target_raw, strip_level)
        if rel:
            rel = _prepend_patch_directory(rel, patch_directory)
        if not rel:
            parse_errors.append(f"Invalid patch target path: {target_raw}")
            continue

        target_path = _patch_rel_path(root, rel)
        existed = False
        doc: _DumpItPatchTextDocument
        file_hunk_results: list[DumpItPatchHunkResult] = []

        try:
            existed = target_path.exists() and target_path.is_file()
        except OSError:
            existed = False

        if existed:
            if is_probably_binary(target_path):
                file_hunk_results.append(
                    DumpItPatchHunkResult(
                        file_path=rel,
                        hunk_index=0,
                        status=DUMPIT_PATCH_STATUS_FAILED_NOT_FOUND,
                        detail="target file is binary; use Git apply",
                    )
                )
                results.append(
                    DumpItPatchFileResult(
                        rel_path=rel,
                        ...
                        before_text="",
                        after_text="",
                        hunk_results=tuple(file_hunk_results),
                        action="modify",
                    )
                )
                continue
            doc = _read_dumpit_patch_text_document(target_path)
        elif is_create:
            doc = _new_dumpit_patch_text_document()
        elif is_delete:
            for hunk_index, _hunk in enumerate(patch_file.hunks, start=1):
                file_hunk_results.append(
                    DumpItPatchHunkResult(
                        file_path=rel,
                        hunk_index=hunk_index,
                        status=DUMPIT_PATCH_STATUS_ALREADY_APPLIED,
                        detail="target file is already absent",
                    )
                )
            results.append(
                DumpItPatchFileResult(
            ...
            for hunk_index, _hunk in enumerate(patch_file.hunks, start=1):
                file_hunk_results.append(
                    DumpItPatchHunkResult(
                        file_path=rel,
                        hunk_index=hunk_index,
                        status=DUMPIT_PATCH_STATUS_FAILED_FILE_MISSING,
        ...
        current_lines = list(doc.lines)
        if is_create and existed:
            # A create-file patch must not insert into an unrelated existing file.
            # It is valid only if the resulting NEW block is already present.
            for hunk_index, hunk in enumerate(patch_file.hunks, start=1):
                _start_line, _old_block, new_block = _effective_hunk_blocks(hunk, reverse=reverse)
                new_matches = _find_exact_block(current_lines, new_block)
                if len(new_matches) == 1:
                    file_hunk_results.append(
                        DumpItPatchHunkResult(
                            file_path=rel,
                            ...
                            status=DUMPIT_PATCH_STATUS_ALREADY_APPLIED,
                            line_no=new_matches[0] + 1,
                            detail="create patch target already contains new block",
                        )
                    )
                    ...
                    file_hunk_results.append(
                        DumpItPatchHunkResult(
                            ...
                            hunk_index=hunk_index,
                            status=DUMPIT_PATCH_STATUS_FAILED_AMBIGUOUS,
                            detail=f"new block matched {len(new_matches)} times in existing create target",
                        )
                    )
                else:
                    file_hunk_results.append(
                        DumpItPatchHunkResult(
                            ...
                            hunk_index=hunk_index,
                            status=DUMPIT_PATCH_STATUS_FAILED_NOT_FOUND,
                            detail="create patch target exists but new block was not found",
                        )
                    )
        ...
        action = "delete" if is_delete else ("create" if is_create else "modify")
        results.append(
            DumpItPatchFileResult(
                rel_path=rel,
                target_path=target_path,
...
def detect_dumpit_apply_plan(
    *,
    ...
    ranked.sort()
    best_failed_score = -1
    best_failed_plan: DumpItPatchPlan | None = None
    best_failed_root: Path | None = None
    best_failed_strip_level: int | None = None
    best_failed_patch_directory: str | None = None
    ...
    for neg_score, _root_index, _dir_index, _level_index, root, strip_level, patch_directory in ranked:
        ...
        try:
            ...
            directory_text = f" --directory={patch_directory}" if patch_directory else ""
            attempts.append(f"FAIL cwd={root}{directory_text} -p{strip_level} score={score}: {exc}")
            continue

        marker = "OK" if not plan.failed else "FAIL"
        directory_text = f" --directory={patch_directory}" if patch_directory else ""
        detail = plan.status_summary() if not plan.failed else _dumpit_patch_plan_failure_detail(plan)
        attempts.append(f"{marker} cwd={root}{directory_text} -p{strip_level} score={score}: {detail}")
        if not plan.failed:
    ...
    if best_failed_plan is not None and best_failed_root is not None and best_failed_strip_level is not None:
        raise RuntimeError(
            ...
            f"{(' --directory=' + best_failed_patch_directory) if best_failed_patch_directory else ''} "
            f"-p{best_failed_strip_level} score={best_failed_score}. "
            f"Failure: {_dumpit_patch_plan_failure_detail(best_failed_plan, 8)}"
            + ("\nAttempts:\n" + detail if detail else "")
        )
...
def dumpit_patch_plan_to_diff(plan: DumpItPatchPlan) -> DumpDiffResult:
    ...
    for file_result in plan.files:
        if file_result.existed:
            before_files[file_result.rel_path] = file_result.before_text
        if file_result.after_text is not None:
            after_files[file_result.rel_path] = file_result.after_text
    before = DumpSnapshot(
        source=plan.root / f"__dumpit_apply_before__{plan.patch_path.name}",
        files=before_files,
    )
    after = DumpSnapshot(
        source=plan.root / f"__dumpit_apply_after__{plan.patch_path.name}",
        files=after_files,
    )
    return compare_dump_snapshots(before, after)
...
def execute_dumpit_patch_plan(plan: DumpItPatchPlan) -> tuple[int, int]:
    if plan.failed:
        raise RuntimeError(
            "DumpIt apply is transactional: no files were written because the plan has failed hunks. "
    ...
    for file_result in plan.files:
        if not file_result.changed:
            continue
        if file_result.action == "delete" and file_result.after_text is None:
            if file_result.target_path.exists() and file_result.target_path.is_file():
                file_result.target_path.unlink()
                deleted += 1
            continue
...
class DumpItApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.patch_engine = tk.StringVar(value="dumpit")
        ...
        self.patch_engine.trace_add("write", lambda *_args: self._on_apply_patch_selection_changed())
    ...
    def _configure_tk_theme(self) -> None:
        ...
        try:
            ...
            style.configure("TEntry", padding=(3, 1))
            style.configure("TCombobox", padding=(3, 1))
            style.configure("TCheckbutton", padding=(3, 1))
            style.configure("TRadiobutton", padding=(3, 1))
            style.configure("TNotebook.Tab", padding=(7, 3))
        except tk.TclError:
            pass
        ...
        style.configure("TNotebook.Tab", background=colors["tab_bg"], foreground=colors["muted"], padding=(7, 3), bordercolor=colors["border"])
        style.map(
            "TNotebook.Tab",
    ...
    def _build_ui(self) -> None:
        pad = {"padx": 4, "pady": 2}
        root = ttk.Frame(self)
        root.pack(fill="both", expand=True, padx=4, pady=3)
        ...
        self.nb.pack(fill="both", expand=True, padx=0, pady=(1, 1))
        ...
        # ---------- SHARED LOG ----------
        self.log = tk.Text(root, height=3, wrap="word")
        self.log.pack(fill="x", expand=False, padx=0, pady=(2, 1))
        self.log.configure(state="disabled")

        # Footer
        footer = ttk.Frame(root)
        footer.pack(fill="x", padx=0, pady=(0, 1))
    ...
    def _build_profile_bar(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(bar, text="Profile:").pack(side="left", padx=(0, 3))
        self.cb_profiles = ttk.Combobox(bar, textvariable=self.profile_name, state="readonly", width=20)
        self.cb_profiles.pack(side="left", padx=(0, 3))
        ...
        ttk.Button(bar, text="Reset", command=self._reset_defaults).pack(side="right", padx=(3, 0))
        ttk.Button(bar, text="Save", command=self._save_config).pack(side="right", padx=4)
    ...
    def _build_export_tab(self, parent: ttk.Frame, pad: dict) -> None:
        ...
        ).grid(row=0, column=0, sticky="w", padx=3, pady=1)
        ttk.Label(options, text="Keep old:").grid(row=0, column=1, sticky="e", padx=(8, 3), pady=1)
        ...
        ttk.Button(actions, text="Preview", command=self._preview).pack(side="left", padx=(0, 3))
    ...
    def _current_apply_patch_log_key(self) -> str:
        ...
        return "|".join(
            (
                ...
                str(self.patch_engine.get() or "dumpit"),
            )
        )
    ...
    def _copy_current_apply_patch_log_to_clipboard(self) -> None:
        ...
        header = [
            ...
            f"engine: {self._get_patch_engine_label()}",
            "",
        ]
        if lines:
            text = "\n".join(header + lines)
    ...
    def _apply_profile_to_ui(self, name: str) -> None:
        ...
        try:
            ...
            self.patch_engine.set(self._normalize_patch_engine(s.get("patch_engine", "dumpit")))
    ...
    def _write_ui_to_profile(self, name: str) -> None:
        ...
        self._cp[sec_real]["patch_engine"] = self._normalize_patch_engine(self.patch_engine.get())
    ...
    def _reset_defaults(self) -> None:
        # resetta SOLO i campi della UI (poi "Save" per salvarli nel profilo selezionato)
    ...
    def _build_diff_tab(self, parent: ttk.Frame, pad: dict) -> None:
        ...
        desc.pack(fill="x", padx=4, pady=(2, 1))

        old_frame = ttk.LabelFrame(info, text="Old dump")
        old_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(old_frame, textvariable=self.diff_old_file).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(old_frame, text="Browse…", command=lambda: self._browse_diff_dump(self.diff_old_file, "Select old dump")).pack(side="left", padx=4, pady=3)

        new_frame = ttk.LabelFrame(info, text="New dump")
        new_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(new_frame, textvariable=self.diff_new_file).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(new_frame, text="Browse…", command=lambda: self._browse_diff_dump(self.diff_new_file, "Select new dump")).pack(side="left", padx=4, pady=3)

        output_frame = ttk.LabelFrame(info, text="Delta file")
        output_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(output_frame, textvariable=self.diff_output_file).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(output_frame, text="Choose…", command=self._choose_diff_output).pack(side="left", padx=4, pady=3)

        options = ttk.Frame(info)
        options.pack(fill="x", padx=4, pady=3)
        ttk.Label(options, text="Format:").pack(side="left", padx=(0, 3))
        ...
        details.pack(fill="both", expand=True, padx=4, pady=(3, 5))
        self.lbl_diff_status = ttk.Label(details, text="No diff loaded.", wraplength=800, justify="left")
        self.lbl_diff_status.pack(fill="x", padx=4, pady=3)
    ...
    def _build_apply_patch_tab(self, parent: ttk.Frame, pad: dict) -> None:
        ...
        patch_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(patch_frame, textvariable=self.patch_file).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(patch_frame, text="Browse…", command=self._browse_patch_file).pack(side="left", padx=4, pady=3)

        target_frame = ttk.LabelFrame(info, text="Target folder")
        target_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(target_frame, textvariable=self.patch_target_dir).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(target_frame, text="Current", command=lambda: self._set_patch_target_to_current_project(log=True)).pack(side="left", padx=3, pady=3)
        ttk.Button(target_frame, text="Browse…", command=self._browse_patch_target_dir).pack(side="left", padx=4, pady=3)

        options = ttk.LabelFrame(info, text="Options")
        options.pack(fill="x", padx=4, pady=3)
        opt_grid = ttk.Frame(options)
        opt_grid.pack(fill="x", padx=4, pady=3)

        ttk.Label(opt_grid, text="Engine").grid(row=0, column=0, sticky="w", padx=3, pady=1)
        engine_box = ttk.Combobox(
            opt_grid,
            textvariable=self.patch_engine,
            values=("dumpit", "git"),
            state="readonly",
            width=9,
        )
        engine_box.grid(row=0, column=1, sticky="w", padx=3, pady=1)
        ...
        actions = ttk.Frame(info)
        actions.pack(fill="x", padx=4, pady=3)
        ttk.Button(actions, text="Preview", command=self._preview_patch_changes).pack(side="left", padx=4)
        ...
        ttk.Button(actions, text="Copy log", command=self._copy_current_apply_patch_log_to_clipboard).pack(side="left", padx=4)
    ...
    def _normalize_patch_engine(self, engine: str | None = None) -> str:
        value = str(engine or self.patch_engine.get() or "dumpit").strip().lower()
        if value in {"git", "git_apply", "git apply"}:
            return "git"
        return "dumpit"

    def _get_patch_engine_label(self) -> str:
        return "Git apply" if self._normalize_patch_engine() == "git" else "DumpIt apply"

    def _append_dumpit_patch_plan_log(self, plan: DumpItPatchPlan, *, prefix: str = "[DumpIt Apply]") -> None:
        for file_result in plan.files:
    ...
    def _build_quota_tab(self, parent: ttk.Frame, pad: dict) -> None:
        ...
        inherited_frame.pack(fill="x", padx=4, pady=3)
        ttk.Label(
            inherited_frame,
            text=(
        ...
        rules_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(rules_frame, textvariable=self.quota_path_rules).pack(fill="x", padx=4, pady=3)
        ...
        examples.pack(fill="x", padx=4, pady=(0, 4))

        report_frame = ttk.LabelFrame(info, text="HTML report file")
        report_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(report_frame, textvariable=self.quota_report_file).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(report_frame, text="Choose…", command=self._choose_quota_report_file).pack(side="left", padx=4, pady=3)
        ...
        actions = ttk.Frame(info)
        actions.pack(fill="x", padx=4, pady=3)
        ttk.Button(actions, text="Open HTML", command=self._open_quota_report).pack(side="left", padx=4)
        ttk.Button(actions, text="Save HTML", command=self._save_quota_report).pack(side="left", padx=4)

        details = ttk.LabelFrame(info, text="Status")
        details.pack(fill="both", expand=True, padx=4, pady=(3, 5))
        self.lbl_quota_status = ttk.Label(details, text="No inspection run.", wraplength=820, justify="left")
        self.lbl_quota_status.pack(fill="x", padx=4, pady=3)
    ...
    def _build_import_tab(self, parent: ttk.Frame, pad: dict) -> None:
        ...
        dump_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(dump_frame, textvariable=self.import_dump_file).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(dump_frame, text="Browse…", command=self._browse_import_dump_file).pack(side="left", padx=4, pady=3)

        target_frame = ttk.LabelFrame(info, text="Target folder")
        target_frame.pack(fill="x", padx=4, pady=3)
        ttk.Entry(target_frame, textvariable=self.import_target_dir).pack(side="left", fill="x", expand=True, padx=4, pady=3)
        ttk.Button(target_frame, text="Current", command=lambda: self._set_import_target_to_current_project(log=True)).pack(side="left", padx=3, pady=3)
        ttk.Button(target_frame, text="Browse…", command=self._browse_import_target_dir).pack(side="left", padx=4, pady=3)
        ...
        self.lbl_import_status.pack(fill="x", padx=4, pady=3)
    ...
    def _build_watch_tab(self, parent: ttk.Frame, pad: dict) -> None:
        ...
        status.pack(fill="x", padx=4, pady=(0, 3))
        ttk.Label(status, text="Status:").pack(side="left")
        self.lbl_watch_status = ttk.Label(status, text="Stopped")
        self.lbl_watch_status.pack(side="left", padx=(4, 8))
        ttk.Label(status, text="Profile locked at start:").pack(side="left")
        self.lbl_watch_profile = ttk.Label(status, text="—")
        self.lbl_watch_profile.pack(side="left", padx=(4, 0))

        opts = ttk.LabelFrame(info, text="Watch options")
        opts.pack(fill="x", padx=4, pady=3)
        grid = ttk.Frame(opts)
        grid.pack(fill="x", padx=4, pady=3)

        ttk.Label(grid, text="Poll ms").grid(row=0, column=0, sticky="w", padx=3, pady=1)
        ttk.Entry(grid, textvariable=self.watch_poll_ms, width=10).grid(row=0, column=1, sticky="w", padx=3, pady=1)
        ttk.Label(grid, text="Quiet ms").grid(row=0, column=2, sticky="w", padx=3, pady=1)
        ttk.Entry(grid, textvariable=self.watch_quiet_ms, width=10).grid(row=0, column=3, sticky="w", padx=3, pady=1)

        ttk.Checkbutton(
            grid,
            text="Run one export immediately on Start",
            variable=self.watch_export_on_start,
        ).grid(row=1, column=0, columnspan=4, sticky="w", padx=3, pady=1)
        ...
        ttk.Button(actions, text="Export now", command=lambda: self._run_export(show_message=True)).pack(side="left", padx=4)
        ...
        self.lbl_watch_last_change.pack(fill="x", padx=4, pady=(2, 1))

        self.lbl_watch_last_export = ttk.Label(details, text="No automatic export yet.", wraplength=760, justify="left")
        self.lbl_watch_last_export.pack(fill="x", padx=4, pady=(0, 4))
    ...
    def _build_batch_tab(self, parent: ttk.Frame, pad: dict) -> None:
        ...
        ttk.Label(
            top,
            text="Select one or more profiles to run. (Batch uses the saved profile settings — click 'Save' if needed.)",
            wraplength=760,
            justify="left",
        ...
        container.pack(fill="both", expand=True, padx=4, pady=3)
        ...
        # Run buttons
        run = ttk.Frame(info)
        run.pack(fill="x", padx=4, pady=(0, 5))
